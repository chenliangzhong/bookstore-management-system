...
public class ProductController extends BaseApiController {
    ...
    // 增
    @PostMapping("/add")
    public Map<String, Object> add(@RequestParam String book_name, @RequestParam String author, @RequestParam BigDecimal price,
                                   @RequestParam String press, @RequestParam String description, @RequestParam int stock,
                                   @RequestParam String image, @RequestParam String status, @RequestParam @DateTimeFormat(pattern="yyyy-MM-dd")Date create_time ,@RequestParam Long category_id) {
        ...
        if (description == null || description.trim().length() == 0) return onBadResp("describe 不能为空");
   ...
   public Map<String, Object> update(@RequestParam Long id, String book_name, String author , String press, BigDecimal price, String image,
                                                    String description, Integer stock, Integer status, @DateTimeFormat(pattern="yyyy-MM-dd") Date create_time , Long category_id)
   {
       ...
       if (description != null && description.trim().length() == 0) return onBadResp("");
